...
This module provides helper functions used across the codebase for
setting random seeds, configuring logging, and device management.
"""
...
def set_seed(seed: int) -> None:
    ...
    (both CPU and CUDA). It also configures PyTorch for deterministic behavior.
    
    Args:
        seed: The random seed to use.
    
    Note:
        Setting CUBLAS_WORKSPACE_CONFIG=:4096:8 environment variable may be
        required for full determinism on CUDA.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def format_parameters(num_params: int) -> str:
    ...
    Returns:
        str: Formatted string (e.g., "270M", "1.2B").
    """
    if num_params >= 1e9:
...
def load_checkpoint(
    ...
    Returns:
        dict: Checkpoint dictionary with all saved items.
    """
